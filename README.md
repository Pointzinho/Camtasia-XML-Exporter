...
4. Converte cada bloco do `.srt` em um marcador de legenda no formato RTF/XMP que o Camtasia Smart Player espera.
5. Insere esses marcadores dentro da track `Captioning` do XML (criando a track se ela não existir).
6. Ativa a exibição de legendas (`captionsenabled="true"`) no XML e no `_player.html`.
...
2. Extraia todo o conteúdo (o executável `CamtasiaXmlExporter.exe`, o `plugin.json` e as bibliotecas `.dll` extraídas) para:
   ...
   ```
3. Reinicie o Subtitle Edit. O plugin aparece no menu **File** ou **Plugins**.
...
7. **Finalização:** Reempacote a pasta SCORM (`.zip`) e suba/atualize no Moodle (ou outro LMS compatível com SCORM 1.3) . Não é necessário salvar o arquivo `.srt` isoladamente, a injeção já ocorreu diretamente no XML.
...
Isso gera um executável standalone (`CamtasiaXmlExporter.exe`) que **não depende** do .NET estar instalado na máquina de destino — por isso o `plugin.json` usa `"executables"` em vez de `"runtime": "dotnet"` (veja Notas técnicas abaixo).
